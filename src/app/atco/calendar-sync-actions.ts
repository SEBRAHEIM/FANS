...
`
    ...
    allSessions.forEach(session => {
        if (session.start_date) {
            ...
            let statusPrefix = session.status === 'completed' ? '✅ ' : session.status === 'in_progress' ? '🔵 ' : '📅 '

            let description = `Personnel: ${title}`
            description += `\\nLocation: ${location}`
            description += `\\nInstructor: ${session.instructor?.full_name || 'TBD'}`
            if (session.notes) description += `\\n\\nNotes: ${session.notes}`
            description += `\\n\\n--\\nGenerated by FANS Portal`
...
DTSTART:${startStr}
DTEND:${endStr}
SUMMARY:${statusPrefix}${title} @ ${location}
LOCATION:${location}
DESCRIPTION:${description}
STATUS:CONFIRMED
TRANSP:OPAQUE
