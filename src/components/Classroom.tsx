...
import { Play, HelpCircle, CheckCircle2, ChevronRight, Lock, Clock, ArrowRight, ArrowLeft, Send } from 'lucide-react'
...
interface ClassroomProps {
    ...
    courseTitle: string
    modules: Module[]
    initialProgress: string[] // List of completed module IDs
}

export default function Classroom({ courseId, courseTitle, modules, initialProgress }: ClassroomProps) {
    const router = useRouter()
    const [activeModuleIndex, setActiveModuleIndex] = useState(0)
    const [completedModules, setCompletedModules] = useState<string[]>(initialProgress)
                    ...
                    {activeModule.module_type === 'video' ? (
                        <div className="space-y-8">
                            <div className="aspect-video bg-black rounded-3xl overflow-hidden shadow-2xl relative border border-zinc-800">
                                ...
                                />
                                {!isModuleCompleted && (
                                    <div className="absolute top-6 left-6 bg-orange-600 text-white text-[10px] font-black px-4 py-2 rounded-full uppercase tracking-widest shadow-xl flex items-center gap-2">
                                        <Clock className="w-3 h-3" />
                                        Unskippable: Finish video to progress
                                    </div>
                                )}
                            </div>
